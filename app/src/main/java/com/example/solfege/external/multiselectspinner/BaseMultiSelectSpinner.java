...
public abstract class BaseMultiSelectSpinner extends AppCompatSpinner implements
    ...
    protected String allCheckedText = getContext().getString(R.string.all);
    protected String allUncheckedText = getContext().getString(R.string.unselected);
    ...
    /**
     * Construct a new spinner with the given context's theme, the supplied attribute set,
     * and default style. <code>mode</code> may be one of  or
    ...
    public BaseMultiSelectSpinner(
        ...
        @SuppressLint("Recycle") final TypedArray a = context.obtainStyledAttributes(
    ...
    public <T extends BaseMultiSelectSpinner> T setSelectAll(boolean selectAll) {
        if (this.selectAll != selectAll) {
            ...
            if (selected != null) {
                Arrays.fill(selected, selectAll);
                ArrayAdapter<String> spinnerAdapter = new ArrayAdapter<>(getContext(),
                        spinnerItemLayout, new String[]{(isSelectAll()) ? allCheckedText : allUncheckedText});
                setAdapter(spinnerAdapter);
            }
...
//    }
    ...
    public <T extends BaseMultiSelectSpinner> void setSelect(List<Integer> selectedItems) {
        if (selectedItems != null && selectedItems.size() != 0) {
            if (selectedItems.get(selectedItems.size() - 1) >= selected.length) {
                throw new ArrayIndexOutOfBoundsException("Item number is more than available items");
            }
            int count = 0;
            for (int i = 0; i < selected.length; i++) {
                if (selectedItems.get(count) == i) {
                    selected[i] = true;
    ...
    public <T extends BaseMultiSelectSpinner> void selectItem(int selectedItem) {
        if (selectedItem >= selected.length) {
            throw new ArrayIndexOutOfBoundsException("Item number is more than available items");
        }
