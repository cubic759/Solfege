package com.example.solfege.external.FM_Score;
...
class FM_SoundPool {
    ...
    FM_SoundPool(Context context) {
        ...
        sndPool.setOnLoadCompleteListener((soundPool, sampleId, status) -> {
            if (status == 0) {
                ...
                if (FM_ScorePlayer.getInstance(context).SoundsLoaded == 100)
                    FM_ScorePlayer.getInstance(context).SoundsLoadedCDL.countDown();
            }
        });
    ...
    static int GetDurationInMs(@FM_DurationValue int duration, int tupletSize, int tempo, int time_signature_d) {
        ...
        if (time_signature_n == 6 || time_signature_n == 9 || time_signature_n == 12)
